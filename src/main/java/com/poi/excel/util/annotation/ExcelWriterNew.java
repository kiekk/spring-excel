...
public class ExcelWriterNew {
    ...
    public ExcelWriterNew(Workbook workbook, Map<String, Object> data, HttpServletResponse response) {
    ...
    // 엑셀 바디 생성
    private void createBody(Sheet sheet, List<List<String>> bodyList) throws IOException {
        int rowSize = bodyList.size();
        for (int i = 0; i < rowSize; i++) {
            createRow(sheet, bodyList.get(i), i + 1);

            if (sheet instanceof SXSSFSheet && (rowSize % 1_000) == 0) {
                ((SXSSFSheet) sheet).flushRows(1_000);
            }
        }
    }
